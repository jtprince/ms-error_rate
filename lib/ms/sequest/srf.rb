...
class Ms::Sequest::Srf::Header
  ...
  # takes the null padded enzyme string and searches for a carriage return.
  # If so, we are dealing with the wacked out format.
  def is_alternate_srf_header?(enzyme_string)
    enzyme_string.include?("\r")
  end
  ...
  def from_io(fh)
    ...
    [:enzyme, :ion_series, :model, :modifications, :raw_filename, :db_filename, :dta_log_filename, :params_filename, :sequest_log_filename].each do |param|
      string = get_null_padded_string(fh, byte_length[param])
      puts "BEFORE: "
      p string
      if param == :enzyme
        @alternate_header = is_alternate_srf_header?(string)
      end
      chars_re = Regexp.escape( "\r\0" )
      string = string[ 0, string.index(/[#{chars_re}]/) ] if @alternate_header && string != ""
      puts "AFTER"
      p string
      send("#{param}=".to_sym, string )
    end
    self
  end
